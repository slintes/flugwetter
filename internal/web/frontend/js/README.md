...
**`viewport.js`, `barbs.js`, `time.js` and `vfr-penalties.js` must not import Chart.js or
touch the DOM at load time.** That is what lets `internal/web/jstest/` run them under `node --test`. The
