...
- The background shading is a `backgroundBands` plugin drawing on **`beforeDatasetsDraw`** — behind the data, not over it — with positions from `scales.x` so it pans and zooms with everything else. The night intervals come from the backend bounded by civil twilight, the same boundary `scoreVFR` zeroes an hour against, so the grey band and the zero score cannot disagree. `bands.js` holds the intervals and the clipping and must stay free of the DOM.
- Three bands share that plugin, and **precedence is night > restricted > day, applied by subtraction in `setBands`, not by draw order**. All three fills are translucent, so an overlap would blend into a fourth colour exactly where a reader needs an unambiguous answer. Adding a band means subtracting it from what it loses to, not painting it later.
- The green band (`DAY_START_HOUR`..`DAY_END_HOUR`, local) and the red ED-R band are drawn **only at the home airfield** — `isHomeAirport()` in `airports.js`, compared against `appConfig.default_airport`. The green window is a personal flying habit and the red one is the area around EDWN; neither says anything about another field. `bands.js` knows nothing about the selection: `api.js` passes the gate in, which is what keeps it testable under `node --test`.
...
- The green band is `DAY_START_HOUR`..`DAY_END_HOUR` in **local** time, computed in the browser — a fixed window, the same for every airfield, and deliberately *not* anyone's opening hours (those are text under the picker, because sunset caps and lunch breaks are not expressible as a rectangle). Local because the x axis is local, so the band lands under its own tick; the cost is that it moves an hour in UTC at the daylight-saving change, which is what the `dayBands` DST test pins.
- **Night is subtracted from day, not drawn over it.** Both fills are translucent, so an overlap would blend into a third colour exactly on a winter afternoon, where a reader most needs to know which applies. `setBands` does the subtraction, which is also what makes the plugin's draw order irrelevant.
